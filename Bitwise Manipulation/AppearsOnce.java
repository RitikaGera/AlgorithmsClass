...
public class AppearsOnce{
	...
	public static int appearsOnce(int[] arr){
		...
		for( int i = 0; i < arr.length; i++){
			...
			twos |= (ones & x);
			ones ^= x;
			not_threes = ~( ones & twos); // converting common ones of ones and twos to zeroes
			ones &= not_threes; // : if element occurs thrice, remove from ones and twos
			twos &= not_threes;
		}
		return ones;
	}
